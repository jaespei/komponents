 ...
 * TCP Forwarder. 
 *
 * Forwards inbound connections to a given TCP gateway. The forwarder
 * injects a preamble to the connection where the real destination is 
 * specified.
 * 
 */
class TcpForwarder {
    ...
    /**
     * Initializes a new TcpForwarder.
     *
     * @param {Object} opts - Configuration options.
     * @param {string} opts.addr - The forwarder binding address
     * @param {number} opts.port - The forwarder binding port
     * @param {string} opts.gwAddr - The gateway address
     * @param {number} opts.gwPort - The gateway port
     * @param {string} opts.dstAddr - The destination address
     * @param {number} opts.dstPort - The destination port
     */
    constructor(opts) {
        this.log = opts.log || ((msg) => console.log(`[TcpForwarder] ${msg}`));
        this.error = opts.error || error;
        this.log('TcpForwarder(' + JSON.stringify(opts) + ')');
        this.cfg = opts;
    }
    ...
    async stop(cb) {
        ...
        if (!this._started) throw new Error('Unable to stop forwarder: it is not running');
    ...
    _addConnection(src, cb) {
        ...
        // - Create header
        var header = [`${this.cfg.gwAddr}${PORT_SEPARATOR}${this.cfg.gwPort}`];

        // - Insert real destination
        header.push(`${this.cfg.dstAddr}${PORT_SEPARATOR}${this.cfg.dstPort}`);
        ...
        this._connections[locator].srcAddr = src.remoteAddress;
        this._connections[locator].srcPort = src.remotePort;
                    ...
                    if (err) deferred.reject(this.error(err));
                        ...
                        this._read(dst, (err, str) => {
                            if (err) deferred.reject(error(err));
                                ...
                                else deferred.reject(this.error('Unable to open connection against destination: destination refused'));
                            }
                        });
                    }
                });
