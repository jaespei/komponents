...
export default {
  ...
  computed: {
    mode() {
      let labels = _.filter(this.deployment.labels, (label) => {
        if (label.startsWith("perm=")) {
          let [name, perm] = label.split("=");
          let [role, type] = perm.split(":");
          return (
            this.$root.user.roles.includes(role) && (type == "w" || type == "o")
          );
        }
        return false;
      });

      if (labels.length) return "write";
      else return "read";
    },
    labels() {
      return _.map(this.deployment.labels, (label, i) => {
  ...
  methods: {
    log(msg) {
      this.$util.log(`[DeploymentLabels] log(${msg})`);
    },
    clearLabel() {
    ...
    addLabel() {
      this.log(`addLabel()`);
      let labels = this.deployment.labels.concat(
    ...
    removeLabel(label) {
      ...
      let labels = _.filter(this.deployment.labels, (lbl, i) => i != label.id);
