...
export default {
  ...
  computed: {
    mode() {
      let labels = _.filter(this.domain.labels, (label) => {
    ...
    labels() {
      return _.map(this.domain.labels, (label, i) => {
  ...
  methods: {
    log(msg) {
      this.$util.log(`[DomainLabels] log(${msg})`);
    },
    clearLabel() {
    ...
    addLabel() {
      this.log(`addLabel()`);
      let labels = this.domain.labels.concat(
    ...
    removeLabel(label) {
      ...
      let labels = _.filter(this.domain.labels, (lbl, i) => i != label.id);
