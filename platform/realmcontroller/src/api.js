...
class RealmAPI {
    ...
    */
    async removeResource(token, domainId, resourceId) {
        ...
        try {
            let { data: result } = await axios.delete(
                `http://${this.opts.addr}:${this.opts.port}/domains/${domainId}/resources/${resourceId}`,
                { headers: { "Authorization": `Bearer ${token}` } }
            );
            return result;
        } catch (err) {
    ...
    async removeDeployment(token, instanceId) {
        ...
        try {
            let { data: result } = await axios.delete(
                `http://${this.opts.addr}:${this.opts.port}/deployments/${instanceId}`,
                { headers: { "Authorization": `Bearer ${token}` } }
            );
            return result;
        } catch (err) {
