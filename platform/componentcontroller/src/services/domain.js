...
class DomainService {
    ...
    constructor(opts) {
        ...
        this.log = opts.log || (msg => console.log(`[DomainService] ${msg}`));
    }
    ...
    async removeDomain(domainId) {
        ...
        try {
            await axios.delete(`http://${this.opts.addr}:${this.opts.port}/domains/${domainId}`);
        } catch (err) {
            this._error(err);
        }
    }
    ...
    async removeResource(resourceId) {
        ...
        try {
            await axios.delete(`http://${this.opts.addr}:${this.opts.port}/resources/${resourceId}`);
        } catch (err) {
            this._error(err);
        }
    }
...
module.exports = (services, store, utils, opts) => {
    let _opts = {};
    _opts.addr = opts.domainAddr;
    _opts.port = opts.domainPort;
    if (!_opts.addr) _opts.host = process.env.DOMAIN_HOST || "127.0.0.1:10000";
    return new DomainService(_opts);
}