<template>
  <div
    v-if="path == '' || components[path].type == 'basic'"
    class="container"
    ref="container"
  >
    <div
      ...
      @dblclick="mouseEvent('dblClick', $event)"
    ></div>
  </div>

  <div v-else class="container" ref="container">
    <div class="header">Composite!!</div>
    <div
      class="paper"
      ref="paper"
      @pointermove="mouseEvent('move', $event)"
...
export default {
  ...
  data() {
    return {
      ...
      background: {
        //color: "antiquewhite",
        color: this.path == "" ? "white" : "lightgray",
      },
      gridSize: 10,
      drawGrid: {
        name: "mesh",
  ...
  mounted() {
    ...
    this.paper.on("cell:pointerdblclick", () => console.log("Double click!!"));
  ...
  methods: {
    ...
    load(opts) {
      console.log(
        `[ComponentModelViewer] load(${this.path},${JSON.stringify(opts)})`
      );
     ...
     */
    save() {
      console.log(`[ComponentModelViewer] save()`);
    },
    doLayout() {
      console.log(`[ComponentModelViewer] doLayout()`);

      let root = this.components[this.currentPath];
      let layout = this.layout && this.layout[this.currentPath];
        ...
        if (layout.subcomponents) {
          layout.subcomponents.forEach((subcomp) => {
            ...
            subcomp.model = this.components[
              `${this.currentPath}${this.pathSeparator}${subcomp.type}`
            ];
    ...
    mouseEvent(type, ev) {
      //console.log(`[ComponentModelViewer] mouseEvent(${type})`);
      let methodName = `${this.action}${type
        .charAt(0)
    ...
    pointerDblClick(ev) {
      ...
      if (this.actionContext.overCell && this.action == "pointer") {
        console.log("ello");
        /*let path = `${this.currentPath}${this.pathSeparator}${this.overCell.get('type')}`;
