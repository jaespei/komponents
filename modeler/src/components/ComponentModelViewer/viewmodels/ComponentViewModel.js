...
var ComponentViewModel = joint.shapes.basic.Generic.extend({
    ...
    portMarkup: [
        ...
        '<polygon class="port-body" points="-10,10 0,-10 10,10"/>',
    ...
    initialize: function(opts) {
        //common.log('[ComponentViewModel] initialize(' + JSON.stringify(opts) + ')');
        opts = opts || {};
...
*/
        ...
        //common.log('[ComponentViewModel] ports=' + this.getPorts());
     ...
     */
    cleanUp: function() {
        //common.log('[ComponentViewModel] cleanUp()');
        //var self = this;
    ...
    header: function(header) {
        //common.log('[ComponentViewModel] header(' + JSON.stringify(header) + ')');
        ...
        if (!header) {
            ...
            if (header.title) {
                ...
                attrs['.header']['fill'] = header.fill || common.COLOR_BACKGROUND;
                ...
                attrs['.header-text'] = {
                    ...
                    'ref-y': 10,
                    //'y-alignment': 'middle',
                    fill: header.title.fill || common.COLOR_FOREGROUND,
                ...
                common.log(this.get('size').width - header.icons.length * (common.DESIGN_ICON_WIDTH + 2));
                if (header.icons.length) attrs['.truncate-header'].width = this.get('size').width - header.icons.length * (common.DESIGN_ICON_WIDTH + 2) - 10;
                else attrs['.truncate-header'].refWidth = -10; // 5(left) 5(right)

                // - define truncate handler
                /*if (header.icons.length) {
                    var self = this;
                    header.cb = function(model, val, opts) {
                        //common.log('change:size');
            ...
            _.each(header.icons, function(icon, i) {
                ...
                attrs['#header-icon-' + i] = {
                    ...
                    fill: icon.fill || common.COLOR_FOREGROUND
                };
                if (i === 0) {
     ...
     */
    body: function(body) {
        //common.log('[ComponentViewModel] body(' + JSON.stringify(body) + ')');
        var self = this;
        ...
        if (!body) {
            ...
            // body style
            attrs['.body']['fill'] = body.fill || common.COLOR_BACKGROUND;
            attrs['.body']['stroke'] = body.stroke || common.COLOR_FOREGROUND;
            attrs['.body']['stroke-width'] = 1;
            ...
            if (body.title) {
                ...
                attrs['.body-text'] = {
                    ...
                    /*xAlignment: 'middle',*/
                    yAlignment: 'middle',
                    fill: body.title.fill || common.COLOR_FOREGROUND,
            ...
            if (body.icon) {
                ...
                attrs['#body-icon'] = {
                    ...
                    xAlignment: 'middle',
                    yAlignment: 'middle',
                    fill: body.icon.fill || common.COLOR_FOREGROUND,
                    'clip-path': 'url(#truncate-body-' + self.cid + ')'
                };
            }
    ...
    footer: function(footer) {
        //common.log('[ComponentViewModel] footer(' + JSON.stringify(footer) + ')');
        var self = this;
        ...
        if (!footer) {
            ...
            attrs['.footer']['fill'] = footer.fill || common.COLOR_BACKGROUND;
            attrs['.footer-text'] = {
                ...
                'y-alignment': 'bottom',
                'font-size': '0.75em',
                fill: footer.title.fill || common.COLOR_FOREGROUND,
    ...
    state: function(value) {
        //common.log('[ComponentViewModel] state(' + value + ')');
     ...
     * @param {string} [opts.color] - The highlighting color
    ...
    highlight: function(opts, value) {
        ...
        /*let color = self.get("color") || common.COLOR_FOREGROUND;
        let darkColor = common.colorLuminance(color, -0.5);*/
        ...
        if (_.isBoolean(value)) {
            ...
            if (opts.type === 'select') {
                if (value) {
                    // set selector 
                    self.attr('.topleft/fill', opts.color || common.COLOR_HIGHLIGHT);
                    self.attr('.topright/fill', opts.color || common.COLOR_HIGHLIGHT);
                    self.attr('.bottomright/fill', opts.color || common.COLOR_HIGHLIGHT);
                    self.attr('.bottomleft/fill', opts.color || common.COLOR_HIGHLIGHT);
            ...
            /*else if (opts.type === 'strong') {
                           self.attr('.body',
                               (value ? { stroke: color, 'stroke-width': 2 } : {
                                   stroke: darkColor,
                                   'stroke-width': 1
                               }));
                       }*/
    ...
    addPort: function(port) {
        //common.log('[ComponentViewModel] addPort(' + JSON.stringify(port) + ')');
        var self = this;
        if (!port) throw new Error('Parameters missing');

        /*let color = self.get("color") || common.COLOR_FOREGROUND;
        let darkColor = common.colorLuminance(color, -0.5);
        let lightColor = self.get("color") ? common.colorLuminance(self.get("color"), 0.5) : common.COLOR_BACKGROUND;*/
        ...
        port.attrs = {
            '.port-label': {
                text: port.name,
                fill: port.color || self.attr(".header-text/fill") || common.COLOR_FOREGROUND
            },
            /*'.port-text': {
                text: port.type || '',
            ...
            '.port-body': {
                stroke: port.border || self.attr(".body/stroke") || common.COLOR_FOREGROUND
            }
        };
        if (port.type == "in" && port.position == "left") port.attrs['.port-body'].transform = "rotate(90)";
        else if (port.type == "in" && port.position == "right") port.attrs['.port-body'].transform = "rotate(-90)";
        else if (port.type == "out" && port.position == "right") port.attrs['.port-body'].transform = "rotate(90)";
        else if (port.type == "out" && port.position == "left") port.attrs['.port-body'].transform = "rotate(-90)";

        port.attrs['.port-body'].fill = port.background || self.attr(".body/fill") || common.COLOR_BACKGROUND;
        //port.attrs['.port-text'].fill = port.color || darkColor;
        port.attrs['.port-body'].stroke = port.border || self.attr(".body/stroke") || common.COLOR_FOREGROUND;
    ...
    updatePort: function(port) {
        //common.log('[ComponentViewModel] updatePort(' + JSON.stringify(port) + ')');
        var self = this;
        if (!port) throw new Error('Parameters missing');
        if (!port.id) throw new Error('Port id missing');

        var current = self.getPort(port.id);
        if (!current) return; //throw Error('Port \'' + port.id + '\' not found');
    ...
    removePort: function(id) {
        //common.log('[ComponentViewModel] removePort(' + id + ')');
        var self = this;
    ...
    resetPorts: function(ports) {
        //common.log('[ComponentViewModel] resetPorts(' + JSON.stringify(ports) + ')');
        var self = this;
        ...
        _.each(self.getPorts(), function(port, i) {
            ...
            if (found) {
                ...
                self.updatePort({
                    ...
                    name: found.name,
                    type: found.type,
                    position: found.position || port.position || null,
                    background: found.background || port.background,
                    color: found.color || port.color,
                    border: found.border || port.border
                });
    ...
    highlightPort: function(id, opts, value) {
        //common.log('[ComponentViewModel] highlightPort(' + id + ',' + JSON.stringify(opts) + ',' + value + ')');
        var self = this;
    ...
    _updatePortHighlight: function(id) {
        //common.log('[ComponentViewModel] _updatePortHighlight(' + id + ')');
        var self = this;
        var hs = self.portProp(id, 'highlighters');
        ...
        self.portProp(id, 'attrs/.port-body', {
            stroke: (h && h.color || self.attr(".body/stroke") || common.COLOR_FOREGROUND),
            'stroke-width': h ? 2 : 1
        });

        // - for published endpoints
        h = _.find(hs, function(_h) { return _h.type === 'strong'; });
        self.portProp(id, 'attrs/.port-body/fill', (h && h.color || self.attr(".body/fill") || common.COLOR_BACKGROUND));
        //self.portProp(id, 'attrs/.port-text/fill', (h ? h.text || lightColor : darkColor));
    ...
    _updatePositions: function(position) {
        //common.log('[ComponentViewModel] _updatePositions(' + position + ')');
        var self = this;
    ...
    highlightIcon: function(id, opts, value) {
        //common.log('[ComponentViewModel] highlightIcon(' + id + ',' + JSON.stringify(opts) + ',' + value + ')');
        var self = this;
        ...
        if (_.isBoolean(value)) {
            ...
            if (opts.type === 'light') {
                ...
                if (value) {
                    self.attr('#body-icon-' + id + '/stroke', 2);
                    if (opts.color) {
                        self.iconFill = self.attr('#body-icon-' + id + '/fill');
                        self.attr('#body-icon-' + id + '/fill', opts.color);
                    }
                } else {
                    self.attr('#body-icon-' + id + '/stroke', 1);
                    if (self.iconFill) {
                        self.attr('#body-icon-' + id + '/fill', self.iconFill);
                        delete self.iconFill;
                    }
                }
            }
        } else {
            // return highlighter state
            if (opts.type === 'light')
                return self.attr('#body-icon-' + id + '/stroke') === 2;
        }
