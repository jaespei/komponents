...
sed -i "s/<Environment name=\"imagePort\" value=.*/<Environment name=\"imagePort\" value=\"${IMAGE_PORT}\"/g" /usr/local/tomcat/conf/context.xml
sed -i "s/<Environment name=\"recommenderHost\" value=.*/<Environment name=\"recommenderHost\" value=\"${RECOMMENDER_DNS}\"/g" /usr/local/tomcat/conf/context.xml
sed -i "s/<Environment name=\"recommenderPort\" value=.*/<Environment name=\"recommenderPort\" value=\"${RECOMMENDER_PORT}\"/g" /usr/local/tomcat/conf/context.xml
sed -i "s/<Environment name=\"persistenceHost\" value=.*/<Environment name=\"persistenceHost\" value=\"${PERSISTENCE_DNS}\"/g" /usr/local/tomcat/conf/context.xml
